# cryrcit/ubuntu-light-baseimage

[![Docker Pulls](https://img.shields.io/docker/pulls/cryrcit/ubuntu-light-baseimage.svg)][hub]
[![Docker Stars](https://img.shields.io/docker/stars/cryrcit/ubuntu-light-baseimage.svg)][hub]
[![](https://images.microbadger.com/badges/image/cryrcit/ubuntu-light-baseimage.svg)](http://microbadger.com/images/cryrcit/ubuntu-light-baseimage "Get your own image badge on microbadger.com")

[hub]: https://hub.docker.com/r/cryrcit/ubuntu-light-baseimage

Latest release: 22.04 [Changelog](CHANGELOG.md)
 | [Docker Hub](https://hub.docker.com/cryrcit/ubuntu-light-baseimage/) 

This project is generated from the original project of osixia if you want take the original please see this link : [Osixia](https://github.com/osixia/docker-light-baseimage)

A **Ubuntu 22.04 (Jammy)** based docker image to build reliable image quickly. This image provide a simple opinionated solution to build multiple or single process image with minimum of layers and an optimized build.
...
Other base distribution are available (Osixia repository):
...
Table of Contents
- [cryrcit/ubuntu-light-baseimage](#cryrcitlight-baseimage)
...
- Help new users with [issues](https://github.com/cryrcit/ubuntu-openldap/issues) they may encounter
...
Yes in this case it's not really useful to have a secret variable like this, but a concrete example can be found in [cryrcit/ubuntu-openldap](https://github.com/cryrcit/ubuntu-openldap) image.
...
In the Dockerfile we are going to:
  ...
  - Define ports exposed and volumes if needed.
        ...
        # Install multiple process stack, nginx and php7.4-fpm and clean apt-get files
        # https://github.com/cryrcit/ubuntu-light-baseimage/blob/stable/image/tool/add-multiple-process-stack
        RUN apt-get -y update \
        ...
        # Use baseimage install-service script
        # https://github.com/cryrcit/ubuntu-light-baseimage/blob/stable/image/tool/install-service
        RUN /container/tool/install-service
...
- [cryrcit/ubuntu-openldap](https://github.com/cryrcit/ubuntu-openldap)

Multiple process images:
- [cryrcit/ubuntu-phpLDAPadmin](https://github.com/cryrcit/ubuntu-phpLDAPadmin)

Image adding light-baseimage tools to an existing image
- [cryrcit/gitlab](https://github.com/cryrcit/ubuntu-gitlab)
...
| Name        | Description |
| ---------------- | ------------------- |
| _runit | Replaces Debian's Upstart. Used for service supervision and management. Much easier to use than SysV init and supports restarting daemons when they crash. Much easier to use and more lightweight than Upstart. <br><br>*This service is part of the multiple-process-stack.*|
| _cron | Cron daemon. <br><br>*This service is part of the multiple-process-stack.*|
| _syslog-ng-core | Syslog daemon so that many services - including the kernel itself - can correctly log to /var/log/syslog. If no syslog daemon is running, a lot of important messages are silently swallowed. <br><br>Only listens locally. All syslog messages are forwarded to "docker logs".<br><br>*This service is part of the multiple-process-stack.* |
| _logrotate | Rotates and compresses logs on a regular basis. <br><br>*This service is part of the multiple-process-stack.*|
| _ssl-tools | Add CFSSL a CloudFlare PKI/TLS swiss army knife. It's a command line tool for signing, verifying, and bundling TLS certificates. Comes with cfssl-helper tool that make it docker friendly by taking command line parameters from environment variables. <br><br>Also add jsonssl-helper to get certificates from json files, parameters are set by environment variables. |
...
Here simple Dockerfile example how to add a service-available to an image:
        ...
        # https://github.com/cryrcit/ubuntu-light-baseimage/blob/stable/image/tool/add-service-available
        # https://github.com/cryrcit/ubuntu-light-baseimage/blob/stable/image/service-available/_ssl-tools/download.sh
        # https://github.com/cryrcit/ubuntu-light-baseimage/blob/stable/image/service-available/_cron/download.sh
        RUN apt-get -y update \
            && /container/tool/add-service-available _ssl-tools _cron \
            && LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends \
               nginx \
               php7.4-fpm
        ...
...
For example a simple image example that add service-available to this baseimage: [cryrcit/ubuntu-web-baseimage](https://github.com/cryrcit/ubuntu-web-baseimage)
...
*Run tool* takes several options, to list them:
    ...
    docker run cryrcit/ubuntu-light-baseimage:22.04 --help
    ...
    cryrcit! Light Baseimage: https://github.com/cryrcit/ubuntu-light-baseimage
...
If a main command is set for example:
    ...
    docker run -it cryrcit/ubuntu-openldap:1.4.0 bash
...
If a main command is set for example:
    ...
    docker run -it cryrcit/phpldapadmin:0.9.0 bash
...
Example:
    ...
    docker run -it cryrcit/ubuntu-light-baseimage:22.04
...
A more complete example can be found [cryrcit/phpLDAPadmin](https://github.com/cryrcit/ubuntu-phpLDAPadmin) image.
...
Can also be set by command line converted in python or json:
    ...
    docker run -it --env FRUITS="#PYTHON2BASH:['orange','apple']" cryrcit/ubuntu-light-baseimage:22.04 printenv
    docker run -it --env FRUITS="#JSON2BASH:[\"orange\",\"apple\"]" cryrcit/ubuntu-light-baseimage:22.04 printenv
